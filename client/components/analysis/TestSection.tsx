...
export default function TestSection() {
  ...
  const handleExportResults = () => {
    ...
    alert.featureUnderConstruction('Results export feature is under construction');
  };
  //// get data from api
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (active) setPlants((result || []).map(plant => ({ id: plant.id.toString(), name: plant.name })));
      } catch (e) {
        if (active) setPlants([]);
      }
    })();
  ...
  const handleCreateTestWorkOrder = async () => {
    ...
    try {
      ...
      const roleDetails = {
        "se-1": { name: "Sarah Chen", title: "Senior Engineer", avatar: "SC", phone: "+91 98765 43210" },
        "je-1": { name: "Priya Sharma", title: "Junior Engineer", avatar: "PS", phone: "+91 98765 43212" },
        "ft-1": { name: "Mohamed Ali", title: "Field Technician", avatar: "MA", phone: "+91 98765 43214" },
      }[testFormData.assignedRole] || { name: "Unknown", title: "Engineer", avatar: "U", phone: "" };

      // Prepare the request body according to the API specification
      const requestBody = {
        Id: `TEST-${Date.now()}`,
        AlertId: "", // Empty for manual test work orders
        Title: `${selectedTemplate.name} - ${plant?.name || "Unknown Site"}`,
        Description: `${selectedTemplate.description}\n\nTest Parameters: ${selectedTemplate.parameters.join(", ")}\nEquipment Required: ${selectedTemplate.equipment.join(", ")}\nCompliance Standards: ${selectedTemplate.compliance.join(", ")}\n\nNotes: ${testFormData.notes || "No additional notes"}`,
        Component: Number(testFormData.siteId), // Using siteId as component
        Type: "test",
        Severity: "medium",
        Status: "InProgress",
        AssignedTo: {
        ...
        EstimatedDuration: selectedTemplate.duration, // Use template duration
        ChecklistProgress: {
          Completed: 0,
          Total: selectedTemplate.parameters.length + 2, // Parameters + setup + cleanup
        },
        ...
        Checklist: [
          {
            Id: `setup-${Date.now()}`,
            Task: "Setup test equipment and safety protocols",
            Completed: false,
            CompletedBy: null,
            CompletedAt: null,
            Notes: null,
          ...
          {
            Id: `cleanup-${Date.now()}`,
            Task: "Cleanup and document test results",
            Completed: false,
            CompletedBy: null,
            CompletedAt: null,
            Notes: null,
        ...
        PlantId: parseInt(testFormData.siteId) || 1,
      };

      console.log('Creating test work order with request body:', requestBody);

      // Make API call to create work order
      const response = await ApiService.post("/workorders", requestBody);
      console.log("Test work order created successfully:", response);

      // Create local work order object for UI
      const newWorkOrder: TestWorkOrder = {
        id: (response as any)?.Id || `TEST-${Date.now()}`,
        ...
        createdBy: "Test Admin",
        createdDate: new Date().toISOString(),
        dueDate: testFormData.dueDate.toISOString(),
        status: "pending",
      ...
      alert.success("Test work order created successfully!");
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Schedule and assign a test execution work order for field personnel
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <Card className="bg-blue-50 border-blue-200">
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    <selectedTemplate.icon className="w-6 h-6 text-blue-600 mt-1" />
                    <div>
                      ...
                      <div className="text-xs text-blue-600">
                        Duration: {selectedTemplate.duration} minutes • 
                        Category: {selectedTemplate.category} • 
                        Standards: {selectedTemplate.compliance.join(", ")}
                      </div>
                    </div>
                  </div>
                </CardContent>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={testFormData.assignedRole} onValueChange={(value) => 
                    setTestFormData(prev => ({ ...prev, assignedRole: value }))
                  }>
                    <SelectTrigger>
                      <SelectValue placeholder="Choose role" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="se-1">Senior Engineer - Sarah Chen</SelectItem>
                      <SelectItem value="je-1">Junior Engineer - Priya Sharma</SelectItem>
                      <SelectItem value="ft-1">Field Technician - Mohamed Ali</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={isCreatingWorkOrder}
            >
              {isCreatingWorkOrder ? "Creating..." : "Create Test Work Order"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
